...
def test_sync_profile_to_resume(tmp_path: Path):
    ...
    profile = UserProfile(
        ...
        professional=ProfessionalInfo(
            ...
            current_company="Municipalidad de Berisso",
    ...
    assert data["personal_information"]["name"] == "Laureano"
    assert data["personal_information"]["surname"] == "Lamonega"
    assert data["personal_information"]["email"] == "test@example.com"
    assert data["personal_information"]["zip_code"] == "1923"
    assert data["education_details"][0]["education_level"] == "Bachelor's"
    assert data["education_details"][0]["institution"] == "UNLP"
    assert data["education_details"][0]["year_of_completion"] == 2027
    assert data["experience_details"][0]["position"] == "DevOps Engineer"
    assert data["experience_details"][0]["company"] == "Municipalidad de Berisso"
