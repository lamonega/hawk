...
async def run_all_tests():
        ...
        <div class="jobs-easy-apply-modal" role="dialog">
            ...
            <div class="fb-dash-form-element">
                ...
                <select id="country-sel">
                    ...
                    <option value="69947580101">United States</option>
                </select>
            </div>
            ...
            <fieldset>
                <legend>Are you legally authorized to work in United States?</legend>
    ...
    res_type = await type_element(li_el["index"], "https://www.linkedin.com/in/example-user")
    assert "Typed" in res_type
    li_val = await page.locator("#li-profile").input_value()
    assert li_val == "https://www.linkedin.com/in/example-user"
    print("  [OK] Input typing verified (value = https://www.linkedin.com/in/example-user)")

    res_ta = await type_element(port_el["index"], "https://github.com/example-user")
    assert "Typed" in res_ta
    port_val = await page.locator("#portfolio-url").input_value()
    assert port_val == "https://github.com/example-user"
    print("  [OK] Textarea typing verified (value = https://github.com/example-user)")

    # --- Test 3: Precision Select ---
    print("\n[TEST 3] Testing precision select dropdown...")
    res_sel = await select_element(country_el["index"], "United States")
    assert "Selected" in res_sel
    sel_val = await page.locator("#country-sel").input_value()
    assert sel_val == "69947580101"
    print("  [OK] Dropdown selection verified (chosen = United States / 69947580101)")
    ...
    assert "linkedin.com" in await page.locator("#li-profile").input_value(), "LinkedIn must be filled"
    assert await page.locator("#country-sel").input_value() == "69947580101", "Country must be United States"
    assert await page.locator("#exp-docker").input_value() == "3", "Docker experience must be 3 years"
