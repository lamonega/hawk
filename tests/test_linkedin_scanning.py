...
def test_spanish_profile_matching():
    p = UserProfile(
        personal={"first_name": "Alex", "last_name": "Taylor", "email": "test@example.com", "phone": "5551234567", "city": "Springfield", "country": "Argentina"},
        work_authorization={"authorized": True, "sponsorship_required": False},
        salary={"expected": "950", "currency": "USD"},
        languages={"english": "Professional"},
    )
    assert match_field("Primer nombre", p) == "Alex"
    assert match_field("Apellido", p) == "Taylor"
    assert match_field("Nombre completo", p) == "Alex Taylor"
    assert match_field("Dirección de correo", p) == "test@example.com"
    assert match_field("Teléfono", p) == "5551234567"
    assert match_field("Ciudad", p) == "Springfield"
