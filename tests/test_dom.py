...
async def test_dom_snapshot_and_interactions(sample_page):
    ...
    # 2. Type into input
    res_type = await type_element(li_input["index"], "https://www.linkedin.com/in/lflamonega")
    assert "Typed" in res_type
    val = await sample_page.locator("#li-profile").input_value()
    assert val == "https://www.linkedin.com/in/lflamonega"

    # 3. Type into textarea
    res_ta = await type_element(port_ta["index"], "https://github.com/lflamonega")
    assert "Typed" in res_ta
    ta_val = await sample_page.locator("#portfolio-url").input_value()
    assert ta_val == "https://github.com/lflamonega"
