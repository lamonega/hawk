...
def test_query_knowledge_base():
    ...
    assert res["direct_facts"]["candidate_name"].startswith("Alex")
    assert res["direct_facts"]["work_authorization"]["country"] == "United States"
...
def test_cover_letter_html_rendering():
    ...
    assert "Alex" in html_en
    assert "alex.taylor@example.com" in html_en
...
def test_recruiter_pitch_spanish_and_english():
    ...
    assert "Tech Solutions" in pitch_es
    assert "Ingeniero de Plataforma" in pitch_es
    assert "Alex" in pitch_es
